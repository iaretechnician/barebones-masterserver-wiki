...
### PLEASE READ:  
In my attempts to simplify everything and keep everything as tidy as possible, I made a small error in judgment. If we were truly to do this from scratch, we'd need to re-write several modules that allow the game to be played. For now, we'll make do with copying the modules from the **MasterServer** object from the scene Assets>Barebones>Demos>Quicksetup>Scenes>*MasterAndSpawner* and use it in our **scratchMasterServer** scene.

Once these are copied over, things should work as intended with the following scripts!
